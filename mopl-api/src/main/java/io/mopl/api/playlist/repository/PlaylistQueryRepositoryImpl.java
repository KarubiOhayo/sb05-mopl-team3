...
public class PlaylistQueryRepositoryImpl implements PlaylistQueryRepository {
  ...
  /**
   * keywordLike / ownerIdEqual / subscriberIdEqual 같은 "기본 필터"를 만드는 곳 - 이 메서드가 리스트 조회 / count 조회 둘
   * 다에서 재사용됨
   */
  ...
  /**
   * 커서 페이징 조건 만들기 - DESC면 "더 작은 값"으로 내려가야 다음 페이지 - ASC면 "더 큰 값"으로 올라가야 다음 페이지 - 정렬키가 같은 경우를 대비해 id로
   * tie-breaker
   */
  ...
  /** 요청 파라미터 문자열을 내부 enum으로 변환 - 오타/허용되지 않는 값이면 바로 예외로 알려주기 */
  private enum SortBy {
    UPDATED_AT,
    SUBSCRIBE_COUNT;
